...
export class GoogleImageEditTool extends AbstractTool {
    constructor() {
        ...
        this.description = '使用Google Gemini处理用户的任意图片（或用户的群聊头像），支持编辑图片内容。当用户请求编辑图片/头像时调用此工具，例如“编辑这张风景照”，“把我的头像改成红色”。';
        this.parameters = {
            type: 'object',
            properties: {
                prompt: {
                    type: 'string',
                    description: '用户对图片的处理需求，例如“将图片转为黑白”“分析这张照片的内容”',
                    // examples: ['把这张图片调亮', '把这张图的人物换一件衣服']
                },
                images: {
                    type: 'array',
